import type { MediaSlot, PageDocument } from "@/lib/product-page/types";

// ---------------------------------------------------------------------------
// DEFAULT_PAGE_DOCUMENT — a byte-for-byte copy of the backend's
// volrep-backend/src/lib/product-page/defaults.ts (itself a 1:1
// transcription of what components/product-landing/ProductLanding.tsx
// rendered on 2026-09-01; see docs/product-page-inventory.md).
//
// It is ONLY the fallback: the storefront normally renders the `published`
// document from GET /api/products/:handle/page. This copy is used when the
// backend is unreachable or returns an unrecognised shape, so the product
// page never blanks. The two copies must stay in sync — a change to the
// backing default MUST be mirrored here.
// ---------------------------------------------------------------------------

const ph = (placeholderLabel: string): MediaSlot => ({
  kind: "placeholder",
  imageId: null,
  url: "",
  poster: "",
