import { parseHomepageDocument, type HomepageDocument } from "./schema.js";

// ---------------------------------------------------------------------------
// DEFAULT_HOMEPAGE_DOCUMENT — the current customer-facing homepage,
// transcribed 1:1 from volrep-maroc:
//   - app/page.tsx                     (section set + render order)
//   - components/home/Hero.tsx + HeroProductVisual.tsx
//   - components/home/BestSellers.tsx
//   - components/home/RecoveryPhilosophy.tsx
//   - components/home/RecoverEverywhere.tsx
//   - components/home/WhyVolrep.tsx
//   - components/home/Testimonials.tsx
//   - components/home/FAQ.tsx
//   - components/home/FinalCTA.tsx
//   - components/home/Newsletter.tsx
//   - lib/i18n.ts  (t.home.*, t.promoBar excluded — PromoBar is global chrome)
// captured 2026-09-10 (see volrep-maroc HOMEPAGE AUDIT).
//
// This document is:
//   - the seed for homepage.draft / .published on a fresh install
//   - the storefront fallback when homepage.published is NULL
//
// DO NOT edit the wording here to "improve" it — this is a faithful copy of
// what ships today. Curly apostrophes (U+2019), the non-breaking space
// (U+00A0) before "?", em dashes, star glyphs and the marketing statistics
// are all intentional and must match the live page byte for byte. Content
// changes are a separate future task.
//
// The six image URLs (Recovery Philosophy background + the five Recover
// Everywhere use-case photos) are the exact Shopify-CDN URLs the components
// use today, preserved verbatim as `kind:"url"` slots. Migrating them into
// site_media is a dedicated later step and does NOT belong here.
//
// `eyebrow` holds only the editable label — the fixed "VOLREP™" / "VOLREP™ /"
// prefix is renderer chrome and is not stored (see schema.ts).
// ---------------------------------------------------------------------------

const CDN = "https://cdn.shopify.com/s/files/1/1010/7476/4088/files";

// A `kind:"url"` image slot pointing at an external URL, with alt text.
const urlImage = (url: string, alt: string) => ({
  kind: "url" as const,
  url,
  alt,
...
// An empty placeholder slot (renders the section's own empty-state frame).
const placeholder = (placeholderLabel = "") => ({ kind: "placeholder" as const, placeholderLabel });
...
export const DEFAULT_HOMEPAGE_DOCUMENT: HomepageDocument = parseHomepageDocument(raw);
